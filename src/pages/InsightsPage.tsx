...
export function InsightsPage({ bookmarks }: Props) {
  const insights = useMemo(() => {
    ...
    return {
      ...
      avgReadingTime: bookmarks.length ? Math.round(
        bookmarks.reduce((sum, b) => sum + (b.readingTime || 0), 0) / (bookmarks.length || 1)
      ) : 0,
      completionRate: bookmarks.length ? Math.round(
        (readingStatusCounts.completed / (bookmarks.length || 1)) * 100
      ) : 0
    };
  }, [bookmarks]);

  const getProgressBarWidth = (value: number, max: number) => {
    if (!value || !max || max <= 0 || isNaN(value) || isNaN(max)) return '0%';
